/**
 * Static game data: the building catalogue and the HQ cost curve.
 *
 * This file is data only. Anything that *computes* with this data lives in
 * ./costs.ts or ./effects.ts.
 */
...
/**
 * The HQ is not a placeable building, but it has the same cost curve shape,
 * so it is modelled alongside the building catalogue.
 */
export const HqData = {
  name: "HQ",
  cost: {
...
/** Base village storage before any buildings are placed. */
...
export const BaseHqWoodOutput = 1;

/** Options list for building pickers — derived once, reused everywhere. */
export const BuildingOptions = BuildingNames.map(name => ({ value: name, label: name }));
